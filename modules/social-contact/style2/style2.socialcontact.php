<?php
function SocialContactStyle2Register($style) {
    $style['SocialContactStyle2'] = ['label' => 'Style 2'];
    return $style;
}
add_filter('SocialContactStyle', 'SocialContactStyle2Register');

class SocialContactStyle2 {
    ...
    static public function adminImg(): void {
        Template::img(Url::base(Path::plugin(SCB_NAME).'/assets/images/socialcontact/demo-style2.png'));
    }
    ...
    static public function adminConfigSave($result) {
        $theme  = Request::post('SocialContactStyle2');
        $config = static::config();
    ...
    static public function render(): void {
        ...
        $buttons = SocialContactStyle2::button($config, $title);
